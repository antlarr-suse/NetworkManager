...
static gboolean
_rules_obj_is_present (RulesObjData *obj_data,
                       gboolean *out_set_has_tombstone)
{
	guint32 priority_val = 0;
	RulesData *rules_data;
	gboolean priority_present = TRUE;
	gboolean has_tombstone = FALSE;
	gboolean has_non_tombstone = FALSE;

	c_list_for_each_entry (rules_data, &obj_data->obj_lst_head, obj_lst) {

		_rules_data_assert (rules_data, TRUE);

		if (rules_data->tombstone) {
			has_tombstone = TRUE;
			continue;
		}

		if (has_non_tombstone) {
			if (priority_val > rules_data->priority_val)
				continue;
			if (priority_val == rules_data->priority_val) {
				priority_present = (priority_present || rules_data->priority_present);
				continue;
			}
		} else
			has_non_tombstone = TRUE;

		priority_val = rules_data->priority_val;
		priority_present = rules_data->priority_present;
	}

	if (has_tombstone)
		*out_set_has_tombstone = TRUE;
                         ...
                         gconstpointer user_tag)
{
	...
	if (changed) {
		_LOGD ("routing-rule: track ["NM_HASH_OBFUSCATE_PTR_FMT",%c%u] \"%s\")",
		       nm_hash_obfuscate_ptr (1240261787u, rules_data->user_tag),
...
static void
_rules_data_set_tombstone (NMPRulesManager *self,
                           RulesData *rules_data)
{
	_rules_data_assert (rules_data, TRUE);
                           ...
                           gconstpointer user_tag)
{
	...
	rules_data = _rules_data_lookup (self->by_data, p_obj_stack, user_tag);
	if (rules_data)
		_rules_data_set_tombstone (self, rules_data);
}
...
static void
_rules_delete (NMPRulesManager *self,
               RulesData *rules_data,
               GHashTableIter *h_iter)
{
	...
	if (c_list_length_is (&rules_data->user_tag_lst, 1)) {
#if NM_MORE_ASSERTS
		RulesUserTagData *user_tag_data;
	...
	nm_assert (!c_list_is_empty (&rules_data->obj_lst));
	if (c_list_length_is (&rules_data->obj_lst, 1)) {
#if NM_MORE_ASSERTS
		RulesObjData *obj_data;
...
nmp_rules_manager_sync (NMPRulesManager *self)
{
	...
	if (pl_head_entry) {
		nmp_cache_iter_for_each (&pl_iter, pl_head_entry, &plobj) {
			...
			if (_rules_obj_is_present (obj_data, &any_tombstones))
				continue;
	...
	if (any_tombstones) {
		g_hash_table_iter_init (&h_iter, self->by_data);
		while (g_hash_table_iter_next (&h_iter, (gpointer *) &rules_data, NULL)) {
			if (rules_data->tombstone)
				_rules_delete (self, rules_data, &h_iter);
		}
	}
	...
	g_hash_table_iter_init (&h_iter, self->by_data);
	while (g_hash_table_iter_next (&h_iter, (gpointer *) &rules_data, NULL))
		nm_assert (!rules_data->tombstone);
#endif
}
