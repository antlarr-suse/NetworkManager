                             ...
                             GCancellable *cancellable)
{
	...
	if (!nm_platform_netns_push (platform, &netns)) {
		g_set_error_literal (&error, 1, 0, "couldn't change namespace");
		goto out_idle;
	}

	dirfd = nm_platform_sysctl_open_netdir (platform, ifindex, ifname);
	if (!dirfd) {
		g_set_error_literal (&error, 1, 0, "couldn't open netdir");
		goto out_idle;
	}
	...
	if (   NM_IN_SET (autoprobe, NM_TERNARY_TRUE, NM_TERNARY_FALSE)
		...
		g_set_error (&error, 1, 0, "couldn't set SR-IOV drivers-autoprobe to %d: %s",
